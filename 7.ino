...
{
 Steering_Servo();
                                          //delay for 100ms more responsive turn per push on bluetoth  
}
...
void SensorTurnRight()                                                          // This Function Turns the car 90 degrees to the right based on the current desired heading
{
  StopCar();    
  getCompass();                                                                  // get current compass heading      

  desired_heading = (desired_heading + 25);                                      // set desired_heading to plus 90 degrees 
  if (desired_heading >= 360) {desired_heading = (desired_heading - 360);}       // if the desired heading is greater than 360 then subtract 360 from it
...
void SensorTurnLeft()                                                           // This procedure turns the car 90 degrees to the left based on the current desired heading
{
  StopCar();    
  getCompass();                                                                  // get current compass heading                                                                                  
  //desired_heading = (compass_heading - 90);                                    // set desired_heading to compass value minus 90 degrees

  desired_heading = (desired_heading - 25);                                      // set desired_heading to minus 90 degrees
  if (desired_heading <= 0) {desired_heading = (desired_heading + 360);}         // if the desired heading is greater than 360 then add 360 to it
  while ( abs(desired_heading - compass_heading) >= compass_dev)                 // If the desired heading is more than Compass Deviation in degrees from the actual compass heading then
      {                                                                          // correct direction by turning left or right
    getCompass();                                                                // Get compass heading again during While Loop
...
//****************************************************************************************************************************************************************************    
 ...
 void Steering_Servo()
{
   if (blueToothVal == 3 && blueToothVal == 4) {
    // do nothing can't go in both directions
    Serial.println("both pushed");
  
  } else {
    if(blueToothVal == 4) //check if left pushed
    {
      Serial.println("left high");
      if(pos1 < 120)
      {
        Serial.println("left not at end");
        pos1 += 10; //increase position by 1
        Serial.println(pos1);
        Steering.write(pos1);
        Serial1.print("Steering Angle =");
        Serial1.println(pos1);
      }
    }
    if(blueToothVal == 3) //check if right pushed
    {
      Serial.println("right high");
       if(pos1 > 60)
      {
        Serial.println("right not at end");
        pos1 -= 10; // decrease position by 1
        Serial.println(pos1);
        Steering.write(pos1);
        Serial1.print("Steering Angle =");
        Serial1.println(pos1);
...
//*****************************************************************************************************************************************************************
 void Buzzer()                                       
 {
  ...
  if (buzzer == true) {
    buzzer = false;
    Serial1.print("Buzzer OFF");
    analogWrite(BUZZER, 255);
  }
    else if (buzzer == false) {
    buzzer = true;
    Serial1.print("Buzzer ON");
...
//*****************************************************************************************************************************************************************
 void Low_Light()                                       
 {
  ...
  if (low_light == true) {
   low_light  = false;
    Serial1.print("Low_Light OFF");
    analogWrite(LOW_LIGHT, 255);
  }
    else if (low_light  == false) {
   low_light  = true;
    Serial1.print("Low_Light ON");
...
//*****************************************************************************************************************************************************************
 void High_Light()                                       
 {
  ...
  if (high_light == true) {
   high_light  = false;
    Serial1.print("High_Light OFF");
    analogWrite(HIGH_LIGHT, 255);
  }
    else if (high_light  == false) {
   high_light  = true;
    Serial1.print("High_Light ON");
    analogWrite(HIGH_LIGHT, 0);
  }
...
void Left_Indicator()                                       
 {
  ...
  if (left_indicator == true) {
   left_indicator  = false;
    Serial1.print("LEFT_INDICATOR OFF");
    analogWrite(LEFT_INDICATOR, 255);
  }
    else if (left_indicator  == false) {
   left_indicator  = true;
    Serial1.print("LEFT_INDICATOR ON");
    analogWrite(LEFT_INDICATOR, 0);
  }
...
void Right_Indicator()                                       
 {
  ...
  if (right_indicator == true) {
   right_indicator  = false;
    Serial1.print("RIGHT_INDICATOR OFF");
    analogWrite(RIGHT_INDICATOR, 255);
  }
    else if (right_indicator  == false) {
   right_indicator  = true;
    Serial1.print("RIGHT_INDICATOR ON");
    analogWrite(RIGHT_INDICATOR, 0);
  }
...
void Cooling_Fan()                                       
 {
  ...
  if (cooling_fan == true) {
   right_indicator  = false;
    Serial1.print("COOLING_FAN OFF");
    analogWrite(COOLING_FAN, 255);
  }
    else if (cooling_fan  == false) {
   cooling_fan  = true;
    Serial1.print("COOLING_FAN ON");
